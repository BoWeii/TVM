...
# print(task.print_best("log_files/matmul_2048.json"))
...
def optimized_matmul(n):
    ...
    if n == 32:
        ...
        C_local_k_o, C_local_k_i = s[C_local].split(C_local_k, factor=4)
        ...
        C_x_o, C_x_i = s[C].split(C_x, factor=32)
        C_y_o, C_y_i = s[C].split(C_y, factor=32)
        s[C].reorder(C_x_o, C_y_o, C_x_i, C_y_i)
        s[C_local].compute_at(s[C], C_y_o)
        C_x_o_y_o_fused = s[C].fuse(C_x_o, C_y_o)
        s[C].parallel(C_x_o_y_o_fused)
        s[C_local].pragma(C_local_x_c_o_o_o, "auto_unroll_max_step", 16)
        s[C_local].pragma(C_local_x_c_o_o_o, "unroll_explicit", True)
        s[C_local].vectorize(C_local_y_c_i)
        s[C].vectorize(C_y_i)
    elif n == 64:
        C_x, C_y, C_k = tuple(C.op.axis) + tuple(C.op.reduce_axis)
        C_x_o_i, C_x_i = s[C].split(C_x, factor=2)
        ...
        C_x_o_o_i, C_x_o_i = s[C].split(C_x_o_i, factor=1)
        C_x_o_o_o, C_x_o_o_i = s[C].split(C_x_o_o_i, factor=16)
        C_y_o_i, C_y_i = s[C].split(C_y, factor=16)
        C_y_o_o_i, C_y_o_i = s[C].split(C_y_o_i, factor=1)
        C_y_o_o_o, C_y_o_o_i = s[C].split(C_y_o_o_i, factor=1)
        C_k_o, C_k_i = s[C].split(C_k, factor=4)
        s[C].reorder(C_x_o_o_o, C_y_o_o_o, C_x_o_o_i, C_y_o_o_i, C_k_o, C_x_o_i, C_y_o_i, C_k_i, C_x_i, C_y_i)
        C_x_o_o_o_y_o_o_o_fused_x_o_o_i_fused = s[C].fuse(C_x_o_o_o, C_y_o_o_o, C_x_o_o_i)
        s[C].parallel(C_x_o_o_o_y_o_o_o_fused_x_o_o_i_fused)
        s[C].pragma(C_x_o_o_o_y_o_o_o_fused_x_o_o_i_fused, "auto_unroll_max_step", 64)
        s[C].pragma(C_x_o_o_o_y_o_o_o_fused_x_o_o_i_fused, "unroll_explicit", True)
        s[C].vectorize(C_y_i)
    elif n == 256:
        C_x, C_y, C_k = tuple(C.op.axis) + tuple(C.op.reduce_axis)
        C_local, = s.cache_write([C], "local")
        ...
        C_local_x_c, C_local_y_c, C_local_k = tuple(C_local.op.axis) + tuple(C_local.op.reduce_axis)
        C_local_x_c_o_i, C_local_x_c_i = s[C_local].split(C_local_x_c, factor=4)
        C_local_x_c_o_o_i, C_local_x_c_o_i = s[C_local].split(C_local_x_c_o_i, factor=32)
        C_local_x_c_o_o_o, C_local_x_c_o_o_i = s[C_local].split(C_local_x_c_o_o_i, factor=1)
        C_local_y_c_o_i, C_local_y_c_i = s[C_local].split(C_local_y_c, factor=16)
        C_local_y_c_o_o_i, C_local_y_c_o_i = s[C_local].split(C_local_y_c_o_i, factor=1)
        C_local_y_c_o_o_o, C_local_y_c_o_o_i = s[C_local].split(C_local_y_c_o_o_i, factor=1)
        ...
        C_x_o_o, C_x_o_i = s[C].split(C_x_o_i, factor=1)
        C_y_o_i, C_y_i = s[C].split(C_y, factor=16)
        ...
        C_x_o_o_y_o_o_fused = s[C].fuse(C_x_o_o, C_y_o_o)
        s[C].parallel(C_x_o_o_y_o_o_fused)
        ...
        C_local_x_c, C_local_y_c, C_local_k = tuple(C_local.op.axis) + tuple(C_local.op.reduce_axis)
        C_local_x_c_o_i, C_local_x_c_i = s[C_local].split(C_local_x_c, factor=4)
        C_local_x_c_o_o_i, C_local_x_c_o_i = s[C_local].split(C_local_x_c_o_i, factor=8)
        ...
        s[C_local].reorder(C_local_x_c_o_o_o, C_local_y_c_o_o_o, C_local_x_c_o_o_i, C_local_y_c_o_o_i, C_local_k_o, C_local_x_c_o_i, C_local_y_c_o_i, C_local_k_i, C_local_x_c_i, C_local_y_c_i)
        C_x_o_i, C_x_i = s[C].split(C_x, factor=32)
        C_x_o_o, C_x_o_i = s[C].split(C_x_o_i, factor=8)
        C_y_o_i, C_y_i = s[C].split(C_y, factor=256)
        ...
        C_local, = s.cache_write([C], "local")
        C_local_x_c, C_local_y_c, C_local_k = tuple(C_local.op.axis) + tuple(C_local.op.reduce_axis)
        C_local_x_c_o_i, C_local_x_c_i = s[C_local].split(C_local_x_c, factor=1)
        C_local_x_c_o_o_i, C_local_x_c_o_i = s[C_local].split(C_local_x_c_o_i, factor=128)
        C_local_x_c_o_o_o, C_local_x_c_o_o_i = s[C_local].split(C_local_x_c_o_o_i, factor=1)
        ...
        C_local_k_o, C_local_k_i = s[C_local].split(C_local_k, factor=8)
        s[C_local].reorder(C_local_x_c_o_o_o, C_local_y_c_o_o_o, C_local_x_c_o_o_i, C_local_y_c_o_o_i, C_local_k_o, C_local_x_c_o_i, C_local_y_c_o_i, C_local_k_i, C_local_x_c_i, C_local_y_c_i)
        C_x_o, C_x_i = s[C].split(C_x, factor=128)
        C_y_o, C_y_i = s[C].split(C_y, factor=512)
        s[C].reorder(C_x_o, C_y_o, C_x_i, C_y_i)
        s[C_local].compute_at(s[C], C_y_o)
        C_x_o_y_o_fused = s[C].fuse(C_x_o, C_y_o)
        s[C].parallel(C_x_o_y_o_fused)
